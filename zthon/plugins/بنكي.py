...
@zedub.zed_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@zedub.zed_cmd(pattern="توب الفلوس(.*)")
...
@zedub.zed_cmd(pattern="مسح حسابي(.*)")
...
@zedub.zed_cmd(func=lambda m:"راتب")
...
@zedub.zed_cmd(pattern="انشاء حساب (.*)")
